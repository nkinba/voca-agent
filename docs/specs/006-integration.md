...
- **Model Context Protocol (MCP)** 서버를 구현하여, 외부 AI 에이전트(Claude Desktop, Cursor)가 `voca-agent`의 데이터베이스를 조회할 수 있게 한다.
- 패키지명: `voca-integration`
...
3. (별도 모드) `voca-agent mcp` 명령어로 실행 시, MCP 서버 모드로 진입하여 표준 입출력(STDIO) 대기.

## 5. Agent Instruction
1. `crates/integration`을 생성한다.
2. 템플릿 엔진(`tera`)을 이용해 Obsidian용 `.md` 파일을 생성하는 로직을 짠다.
3. `voca-core`의 모델을 `serde`로 직렬화하여 MCP 프로토콜(JSON-RPC 유사)에 맞춰 출력하는 기본 서버를 구현한다.